...
func NewSecondaryNodeNetworkController(cnnci *CommonNodeNetworkControllerInfo, netInfo util.NetInfo, defaultNetworkGateway Gateway) (*SecondaryNodeNetworkController, error) {
	...
	if util.IsNetworkSegmentationSupportEnabled() && snnc.IsPrimaryNetwork() {
		...
		snnc.gateway, err = NewUserDefinedNetworkGateway(snnc.NetInfo, networkID, node, snnc.watchFactory.NodeCoreInformer().Lister(), snnc.Kube, defaultNetworkGateway)
		if err != nil {
			return nil, err
		}
	}
