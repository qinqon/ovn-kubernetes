...
`ovn-kubernetes/network/virtualization` suite is a child of
`openshift/network/virtualization`, so the existing metal virtualization job
selects the OVN tests alongside the Origin virtualization tests.
...
It registers that source image with index `-1` and resolves its pullspec through
`KUBE_TEST_REPO`, using the same index-free mirror tag as Origin's
`image.LocationFor`. An empty repository uses the source image directly.
Traffic pods and external iperf3 clients use the payload image resolved from
the latest imported tag in the `openshift/network-tools` ImageStream, matching
...
openshift-tests run openshift/network/virtualization \
