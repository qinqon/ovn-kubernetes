...
func TestConfigureIPerf3Pod(t *testing.T) {
	o := &OpenshiftInfraProvider{networkToolsImage: "registry.example.com/network-tools@sha256:abc"}
	original := []string{"bash", "-c", "iperf3 -s; sleep infinity"}
	pod := &corev1.Pod{Spec: corev1.PodSpec{Containers: []corev1.Container{{
		Name: "traffic", Command: original[:2], Args: original[2:],
	}}}}
	if err := o.ConfigureIPerf3Pod(pod); err != nil {
		t.Fatal(err)
	}
	c := pod.Spec.Containers[0]
	if c.Image != o.networkToolsImage || c.Name != "traffic" || !reflect.DeepEqual(c.Args[2:], original) {
		t.Fatalf("image/command override did not preserve the endpoint: %+v", c)
	}
