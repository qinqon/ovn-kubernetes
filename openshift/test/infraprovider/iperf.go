...
func (o *OpenshiftInfraProvider) getNetworkToolsImage() (string, error) {
	if o.networkToolsImage != "" {
		return o.networkToolsImage, nil
	}
	client, err := imageclient.NewForConfig(o.restConfig)
	if err != nil {
		return "", err
	}
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	// Follow Origin's networking/egressip_helpers.go GetDockerImageReference:
	// use the latest imported tag's concrete pullspec, including mirror refs.
	stream, err := client.ImageV1().ImageStreams("openshift").Get(ctx, "network-tools", metav1.GetOptions{})
	if err != nil {
	...
	for _, tag := range stream.Status.Tags {
		if tag.Tag == "latest" && len(tag.Items) > 0 && tag.Items[0].DockerImageReference != "" {
			o.networkToolsImage = tag.Items[0].DockerImageReference
			return o.networkToolsImage, nil
		}
	}
...
func (o *OpenshiftInfraProvider) ConfigureIPerf3Pod(pod *corev1.Pod) error {
	image, err := o.getNetworkToolsImage()
	if err != nil {
		return err
	}
...
func (o *OpenshiftInfraProvider) PrepareIPerf3Container(ctx api.Context, ec api.ExternalContainer) (api.ExternalContainer, error) {
	...
	if err := pullNetworkTools(pullCtx, image, secret.Data[corev1.DockerConfigJsonKey]); err != nil {
		return ec, err
	}
	ec.Image = image
